...
namespace A23A_MultiUtility
{
    internal class TudoJuntoEmisturado: Soraka
    {
        ...
        internal static void Checagem(EventArgs args)
        {
            ...
            if (Eu.IsDead || !AutoSkill["AutoSkill"].Cast<KeyBind>().CurrentValue || Environment.TickCount <= UltCast + 400 || Environment.TickCount <= UltClick + 400) return;
            if (Eu.Level >= Eu.Spellbook.GetSpell(SpellSlot.Q).Level + Eu.Spellbook.GetSpell(SpellSlot.W).Level + Eu.Spellbook.GetSpell(SpellSlot.E).Level + Eu.Spellbook.GetSpell(SpellSlot.R).Level)
            {
                Core.DelayAction(BigodeGrosso, 1100);
            }
            if (AutoSkill["AutoHeal"].Cast<CheckBox>().CurrentValue  || Environment.TickCount >= UltCast + 400 || Environment.TickCount >= UltClick + 400)
            {
                if (Environment.TickCount >= TickR + 400)
                {
                    AmuniçocaPica();
                }
                if (Environment.TickCount >= TickW + 400 && Environment.TickCount >= TickR + 400)
                {
                    PatenteAlta();
                }
            }
            if (!AutoSkill["AttackEnemy"].Cast<CheckBox>().CurrentValue || Environment.TickCount <= UltCast + 400 || Environment.TickCount <= UltClick + 400) return;

            if (Environment.TickCount >= TickQ + 400)
            {
                RespeitaUmoço();
            }
            if (Environment.TickCount >= TickE + 400 && Environment.TickCount >= TickQ + 400)
            {
                PepecaDuMal();
            }
        }
        ...
        internal static void ChecarMinion()
        {
            ...
            if (Environment.TickCount <= UltClick + 500 || Environment.TickCount <= UltCast + 400) return;
        ...
        internal static void ChecarSendoatacado(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsMe)
            {
                ...
                {
                    if (Environment.TickCount <= UltCast + 400 || Environment.TickCount <= UltClick + 300)
                    {
                        args.Process = false;
                    }
                    else
            ...
            if (sender.Distance(Eu) >= 1300 || !sender.IsEnemy || args.Target == null || !args.Target.IsMe) return;
            if (Orbwalker.ActiveModesFlags != Orbwalker.ActiveModes.None)
            {
                Orbwalker.ActiveModesFlags = Orbwalker.ActiveModes.None;
            }
        ...
        internal static void CorteClick(Obj_AI_Base sender, PlayerIssueOrderEventArgs args)
        {
            ...
            {
                if (Environment.TickCount <= UltClick + 500 || Environment.TickCount <= UltCast + 400)
                {
                    args.Process = false;
                    return;
                }
        ...
        internal static void QuemÉoCantor()
        {
            ...
            AutoSkill.Add("AutoSkill", new KeyBind("Enable Auto Up/Use Skill", false, KeyBind.BindTypes.PressToggle, 115));

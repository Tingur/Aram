...
namespace A23A_MultiUtility
{
    internal class TudoJuntoEmisturado : Soraka
    {
        ...
        {
            if (!Eu.ChampionName.Contains("Soraka")) return;
            Hacks.RenderWatermark = false;
            Bootstrap.Init(null);
            Core.DelayAction(PuloDoGato, 1500);
        }
        ...
        internal static void Checagem(EventArgs args)
        {
            ...
            if (Eu.IsDead || !AutoSkill["AutoSkill"].Cast<KeyBind>().CurrentValue ||
                Environment.TickCount <= UltCast + 300 || Environment.TickCount <= UltClick + 400) return;
            if (Eu.Level >=
                Eu.Spellbook.GetSpell(SpellSlot.Q).Level + Eu.Spellbook.GetSpell(SpellSlot.W).Level +
                Eu.Spellbook.GetSpell(SpellSlot.E).Level + Eu.Spellbook.GetSpell(SpellSlot.R).Level)
            {
                Core.DelayAction(BigodeGrosso, 900 + Aleatorio(30,120));
            }
            if (AutoSkill["AutoHeal"].Cast<CheckBox>().CurrentValue || Environment.TickCount >= UltCast + 300 ||
                Environment.TickCount >= UltClick + 400)
            {
                if (Environment.TickCount >= TickR + 300) AmuniçocaPica();
                if (Environment.TickCount >= TickW + 300 && Environment.TickCount >= TickR + 300) PatenteAlta();
            }
            if (!AutoSkill["AttackEnemy"].Cast<CheckBox>().CurrentValue || Environment.TickCount <= UltCast + 300 ||
                Environment.TickCount <= UltClick + 400) return;
            if (Environment.TickCount >= TickQ + 300) RespeitaUmoço();
            if (Environment.TickCount >= TickE + 300 && Environment.TickCount >= TickQ + 300) PepecaDuMal();
        }
        ...
        internal static void ChecarMinion()
        {
            ...
            if (Environment.TickCount <= UltClick + 400 || Environment.TickCount <= UltCast + 300) return;
            Player.IssueOrder(GameObjectOrder.MoveTo,
        ...
        internal static void ChecarSendoatacado(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsMe)
            {
                ...
                {
                    if (Environment.TickCount <= UltCast + 300 || Environment.TickCount <= UltClick + 300)
                    {
                        args.Process = false;
                    }
                    else
            ...
            if (sender.Distance(Eu) >= 1000 || !sender.IsEnemy || args.Target == null || !args.Target.IsMe) return;
            Fugir = true;
            MeAtacou = sender;
        }
        ...
        internal static void CorteClick(Obj_AI_Base sender, PlayerIssueOrderEventArgs args)
        {
            ...
            {
                if (Environment.TickCount <= UltClick + 400 || Environment.TickCount <= UltCast + 300)
                {
                    args.Process = false;
                    return;
                }
        ...
        internal static void AtivarZoom(ValueBase<bool> sender, ValueBase<bool>.ValueChangeArgs args)
        {
            ...
            Camera.SetZoomDistance(Hacks.ZoomHack ? Utility["ValZoom"].Cast<Slider>().CurrentValue : 1800);
        }
        ...
        */
        ...
        {
            A23A = MainMenu.AddMenu("Auto Soraka", "MenuFollow");
            A23A.AddSeparator();
            ...
            */
            AutoSkill = A23A.AddSubMenu("Soraka", "soraka");
            AutoSkill.AddGroupLabel("Auto skill options");
            AutoSkill.Add("AutoSkill",
                new KeyBind("Enable Auto Up/Use Skill", false, KeyBind.BindTypes.PressToggle, 115));
            AutoSkill.AddSeparator();
            AutoSkill.Add("AutoHeal", new CheckBox("AutoHeal."));
            AutoSkill.Add("AttackEnemy", new CheckBox("Auto Q/E enemy in range."));
        }
    }
}